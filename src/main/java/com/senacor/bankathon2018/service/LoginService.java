...
public class LoginService {
    ...
    private void manageAxwaySubscriptions(Credentials credentials, String deviceToken, Try<String> axwayLoginResponse) {
        axwayLoginResponse
                .mapTry(session -> notificationService.subscripeToAxwayNotification(credentials.getUsername(), session, deviceToken))
                .onSuccess(successful -> {
                    if(successful) LOG.info("Subscribing to axway notifications was successful!");
                    else LOG.info("Subscribing to axway notifications was unsuccessful!");
        }).onFailure(throwable -> {
           LOG.error("An error occured while we tried to subscribe to axway notifications", throwable);
        });
    }

    private void manageFigoSubscriptions(Credentials credentials, Try<TokenResponse> figoLoginResponse) {
